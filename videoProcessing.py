...
​
    ...
    if classes[class_scores.argmax()] == "breakup":
        if counter != len(images)-1:
            prev_frame = images[counter-1]
            next_frame = images[counter+1]
            
            prev_and_curr = cv2.addWeighted(prev_frame,0.5,frame,0.5,0)
            next_and_curr = cv2.addWeighted(next_frame,0.5,frame,0.5,0)
            images[counter] = cv2.addWeighted(prev_and_curr,0.5,next_and_curr,0.5,0)   
    
    #add frame to output video using opencv VideoWriter
